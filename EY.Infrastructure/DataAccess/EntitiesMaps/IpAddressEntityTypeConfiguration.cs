...
public class IpAddressEntityTypeConfiguration : IEntityTypeConfiguration<IpAddress>
{
    public void Configure(EntityTypeBuilder<IpAddress> builder)
    {
        ...
        builder.Property(x => x.Ip).HasColumnName(@"IP").HasColumnType("varchar(15)").IsRequired().IsUnicode(false)
            .HasMaxLength(15);
        ...
        builder.HasOne(a => a.Country).WithMany(b => b.IpAddresses).HasForeignKey(c => c.CountryId)
            .OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_IPAddresses_Countries");
