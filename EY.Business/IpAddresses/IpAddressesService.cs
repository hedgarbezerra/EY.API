...
public class IpAddressesService : IIpAddressesService
{
    ...
    public IpAddressesService(IUnitOfWork unitOfWork, IRedisCache redisCache, IIp2CService httpConsumer,
    ...
    public Result Add(IpAddressInput ipAddress)
    {
        ...
        var country = countriesRepository.Get()
            .FirstOrDefault(c => c.ThreeLetterCode == ipAddress.CountryThreeLetterCode);
        if (country is null)
        {
            country = new Country
            {
        ...
        var cacheKey = CachePrefix + ipAddress.IpAddress;
    ...
    public async Task<Result<Ip2CResponse>> Get(string ipAddress, CancellationToken cancellationToken = default)
    {
        ...
        {
            var convertedCachedIp = new Ip2CResponse(ipAddress, cachedIp.Country.Name, cachedIp.Country.TwoLetterCode,
                cachedIp.Country.ThreeLetterCode);
        ...
        if (dbIp is not null)
        {
            ...
            var dbConvertedIp = new Ip2CResponse(ipAddress, dbIp.Country.Name, dbIp.Country.TwoLetterCode,
                dbIp.Country.ThreeLetterCode);
        ...
        var insertionResult = Add(new IpAddressInput(ipAddress, serviceIpResult.Data.CountryName,
            serviceIpResult.Data.CountryTwoLetterCode, serviceIpResult.Data.CountryThreeLetterCode));

        if (!insertionResult.Successful)
            return Result<Ip2CResponse>.Success(serviceIpResult.Data,
                "IP Address found externally, but failed to be saved.");

        dbIp = repository.Get().FirstOrDefault(ip => ip.Ip == ipAddress);

        var ip2CIp = new Ip2CResponse(ipAddress, dbIp.Country.Name, dbIp.Country.TwoLetterCode,
            dbIp.Country.ThreeLetterCode);
    ...
    public Result<PaginatedList<Ip2CResponse>> Get(PaginationInput pagination)
    {
        ...
        var mappedIps = ips.Select(ip =>
                new Ip2CResponse(ip.Ip, ip.Country.Name, ip.Country.TwoLetterCode, ip.Country.ThreeLetterCode))
            .ToList();
        var paginatedIps = new PaginatedList<Ip2CResponse>(mappedIps, pagination.Index, pagination.Size, totalCount);

        return Result<PaginatedList<Ip2CResponse>>.Success(paginatedIps,
            $"A total of {paginatedIps.TotalCount} in {paginatedIps.TotalPages} pages.");
    }

    public Result<List<IpAddressReportItem>> Report(params string[] twoLetterCountriesCodes)
    {
        var query = _sqlExecutor.Query<IpAddressReportItem>(
            @"SELECT TOP 100 PERCENT c.Name AS CountryName, COUNT(ips.Id) AS IpAddressesCount, MAX(ips.CreatedAt) AS LastIpAddressUpdatedAt
                                                                    FROM dbo.IPAddresses ips 
                                                                    INNER JOIN dbo.Countries c
                                                                    ON ips.CountryId = c.Id
                                                                    WHERE c.TwoLetterCode in @CountryCodes
                                                                    GROUP BY c.Name
                                                                    ORDER BY COUNT(ips.Id) DESC, MAX(ips.CreatedAt) DESC",
            new { CountryCodes = twoLetterCountriesCodes });

        return Result<List<IpAddressReportItem>>.Success(query.ToList(),
            $"Report returned a total of {query.Count()} rows.");
    }
    ...
    public Result Update(IpAddressInput ipAddress)
    {
        ...
        var ip = ipRepository.Get().FirstOrDefault(ip => ip.Ip == ipAddress.IpAddress);
        if (ip is null)
