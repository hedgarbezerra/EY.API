...
public static class APIServiceCollectionExtensions
{
    ...
    /// <remarks>
    ///     Must be called before every other bindings or configurations.
        ...
        ConfigurationManager configurationManager)
    {
        ...
        if (!environment.IsProduction() || azureOptions is not
                { AppConfigurations: { ConnectionString: { Length: > 0 } } })
            return services;
        ...
        configurationManager.AddAzureAppConfiguration(config =>
        {
            config.Connect(azureOptions.AppConfigurations.ConnectionString)
                .ConfigureRefresh(opt =>
                {
                    ...
                    opt.SetRefreshInterval(
                        TimeSpan.FromSeconds(azureOptions.AppConfigurations.CacheExpiracySeconds.Value));
                });

            config.Select(KeyFilter.Any);
    ...
    public static IServiceCollection AddAPIVersioning(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        var apiVersion = configuration.GetValue<double>(Constants.Options.Api.VersionKey);
    ...
    public static IServiceCollection AddAPIAuthentication(this IServiceCollection services)
    {
        var authenticationOptions =
            services.BuildServiceProvider().GetRequiredService<IOptions<AuthenticationOptions>>().Value;
    ...
    public static IServiceCollection AddAPIMappedServices(this IServiceCollection services)
    {
        var assemblies = new[]
        {
            typeof(Program).Assembly, typeof(SharedAssemblyBinder).Assembly,
